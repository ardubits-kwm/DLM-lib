...
namespace DLM {
    ...
    //% block="OLED显示第$n行显示字符串 $str"
    //% group="OLED" weight=6
    export function oledlinewriteString(n:number,str: string) {
        charY = n
        charX = xOffset
        for (let i = 0; i < str.length; i++) {
            if (charX > displayWidth - 6) {
    ...
    //% block="OLED第$d行显示数字$n"
    //% group="OLED" weight=5
    export function oledlinewriteNum(d:number,n: number) {
        let numString = n.toString()
        oledlinewriteString(d,numString)
    }
...
//APDS9960-------------------------------------------------------
    ...
    //% blockId=motostop block="%port 电机停止"
    //% weight=63
    //% group="执行器"
    export function Motorstop(port: MOTORNUM){
        if (port == 1){
            pins.analogWritePin(AnalogPin.P15,0)
            pins.analogWritePin(AnalogPin.P16,0)           
        }else if(port == 2){
