...
//% advanced=true
     ...
     */
    //% group="APDS9960"
    //% blockId="Gesture_readGesture" weight=103 blockGap=15
    //% block="读取手势"
     ...
     */
    //% group="APDS9960"
    //% blockId="Gesture_onGesture" weight=102 blockGap=15
    //% block="检测手势 |%u"
     ...
     */
    //% group="APDS9960"
    //% blockId="Gesture_getGestureID" weight=101 blockGap=15
    //% block="获取手势值"
     ...
     */
    //% group="APDS9960"
    //% blockId=Gesture_enable_mode weight=100 blockGap=15
    //% block="初始化手势传感器"
    ...
    //% blockId=dht11 block="DHT11温湿度端口 %port 获取 %readtype"
    //% weight=60
    //% group="Basic"
    export function DHT11(port: DigitalPin, readtype: DHT11Type): number {
        let dht11pin = port
    ...
    //% blockId=Rbled block="双色灯红灯端口%port1状态%iodata1蓝灯端口%port2状态%iodata2"
    //% weight=61
    //% group="Basic"
    ...
    //马达----------------------------------------------------------------------
    //% blockId=moto block="%port 电机%iodata"
    //% weight=62
    //% group="Basic"
    export function Motorset(port: MOTORNUM,iodata:MOTORTYPE){
        if (port == 1){
            if(iodata == 1){
